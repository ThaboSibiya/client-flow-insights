...
import { getFileAccessHistory, FileAccessHistoryEntry } from '@/services/auditLogService';
...
const FileAccessHistoryList = ({ searchTerm }: FileAccessHistoryListProps) => {
  ...
  const filteredHistory = useMemo(() => {
    ...
    return (history || []).filter((log: FileAccessHistoryEntry) => {
  ...
  if (error) return (
    <Alert variant="destructive">
      ...
      <AlertDescription>Failed to load file access history: {(error as Error).message}</AlertDescription>
    </Alert>
  );
