...
BEGIN
  ...
  BEGIN DELETE FROM public.payments WHERE invoice_id IN (SELECT id FROM public.invoices WHERE workspace_id = p_workspace_id); EXCEPTION WHEN OTHERS THEN NULL; END;
  BEGIN DELETE FROM public.invoices WHERE workspace_id = p_workspace_id; EXCEPTION WHEN OTHERS THEN NULL; END;
  BEGIN DELETE FROM public.ticket_comments WHERE ticket_id IN (SELECT id::text FROM public.tickets WHERE workspace_id = p_workspace_id); EXCEPTION WHEN OTHERS THEN NULL; END;
  BEGIN DELETE FROM public.ticket_activities WHERE ticket_id IN (SELECT id FROM public.tickets WHERE workspace_id = p_workspace_id); EXCEPTION WHEN OTHERS THEN NULL; END;
  BEGIN DELETE FROM public.tickets WHERE workspace_id = p_workspace_id; EXCEPTION WHEN OTHERS THEN NULL; END;
  BEGIN DELETE FROM public.followups WHERE customer_id IN (SELECT id FROM public.customers WHERE workspace_id = p_workspace_id); EXCEPTION WHEN OTHERS THEN NULL; END;
  BEGIN DELETE FROM public.meeting_notes WHERE customer_id IN (SELECT id FROM public.customers WHERE workspace_id = p_workspace_id); EXCEPTION WHEN OTHERS THEN NULL; END;
  BEGIN DELETE FROM public.scheduled_calls WHERE customer_id IN (SELECT id FROM public.customers WHERE workspace_id = p_workspace_id); EXCEPTION WHEN OTHERS THEN NULL; END;
  BEGIN DELETE FROM public.customers WHERE workspace_id = p_workspace_id; EXCEPTION WHEN OTHERS THEN NULL; END;

  FOR i IN 1..array_length(v_names, 1) LOOP
    INSERT INTO public.customers (
      workspace_id, user_id, name, email, phone, status, source, created_at
    ) VALUES (
      p_workspace_id, v_owner_id, v_names[i],
    ...
    v_amount := (500 + (i * 137) % 9500)::numeric;

    IF i % 2 = 0 THEN
      BEGIN
        INSERT INTO public.invoices (
          workspace_id, user_id, customer_id, invoice_number,
          amount, total_amount, status, due_date, issue_date
        ) VALUES (
        ...
        IF i % 4 = 0 THEN
          BEGIN
            INSERT INTO public.payments (
              ...
              v_owner_id, v_customer_id, v_invoice_id, v_amount,
    ...
    IF i % 5 != 0 THEN
      BEGIN
        INSERT INTO public.tickets (
          workspace_id, user_id, customer_id, ticket_number,
          subject, description, status, priority, created_at
        ) VALUES (
          p_workspace_id, v_owner_id, v_customer_id,
          'TKT-DEMO-' || lpad(i::text, 4, '0'),
